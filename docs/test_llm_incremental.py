"""
LLM Performance Testing Script for Qwen3 Models - Incremental Token Testing
Tests three specialized models on 192.168.50.15 with token sizes from 128 to 8192:
...
class LLMTester:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    return {
                        ...
                        "type": "chat",
                        "duration": duration,
                        "input_tokens": len(prompt.split()) * 1.33,  # Approximate
    ...
    async def test_reranker(self, session, model_name, query, documents):
        """Test reranker model by scoring documents"""
        config = MODELS[model_name]
        url = f"http://{config['host']}:{config['port']}/v1/completions"
...
async def main():
    print("\n" + "="*80)
    print("Qwen3 Models - Incremental Token Testing (128 to 8192 tokens)")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for tokens in TOKEN_SIZES:
            ...
            max_output = min(200, tokens // 4)  # Generate up to 1/4 of input or 200 max
            ...
            if not result['error']:
                print(f"✓ {result['duration']:.2f}s ({result['tokens_per_second']:.1f} tok/s, {result['output_tokens']} tokens generated)")
            else:
            ...
            # Reranker model
            num_docs = min(5, max(2, tokens // 512))  # 2-5 documents based on token size
    ...
    output_file = f"qwen3_incremental_test_{timestamp}.json"
