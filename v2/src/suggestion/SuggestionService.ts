  ...
  private async deleteOldSuggestions(): Promise<void> {
    try {
      ...
      if (suggestionReviews.length > 0) {
        core.info(`Found ${suggestionReviews.length} old suggestion review(s), dismissing...`);
        
        for (const review of suggestionReviews) {
          try {
            await this.commentService.dismissReview(this.prNumber, review.id, 'Outdated - superseded by newer suggestions');
            core.info(`  ✓ Dismissed review #${review.id}`);
          } catch (error) {
            core.warning(`Could not dismiss review #${review.id}: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
      } else {
