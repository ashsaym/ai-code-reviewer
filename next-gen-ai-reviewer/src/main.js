...
function formatComment({ task, completion, repo, prNumber }) {
  ...
  const header = `### 🤖 Next Gen AI Reviewer\n` +
    `**Task:** ${taskLabel}  |  **Model:** ${completion.model} (${completion.provider})`;

  const footer = `\n\n_Reply generated by next-gen-ai-reviewer v${packageJson.version} for ${repo}#${prNumber}_`;

  return `${header}\n\n${completion.content}${footer}`;
}

async function tryProviders({ providers, prompt, task, models, selfHostedConfig, maxTokens }) {
  const failures = [];
...
async function run() {
  ...
  const body = formatComment({ task, completion, repo: repository, prNumber });
