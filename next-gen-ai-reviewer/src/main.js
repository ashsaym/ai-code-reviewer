...
async function postInlineReview({ token, owner, repo, prNumber, prMetadata, completion, files }) {
  try {
    ...
    const reviewBody = `## 🤖 AI Code Review\n\n${summary}\n\n_Generated by next-gen-ai-reviewer v${packageJson.version} (${completion.model}, ${completion.provider})_`;
...
async function run() {
  ...
  const useInlineReview = getInput("inline-review") !== "false" && TASK_LIBRARY[task]?.inline;
